...
getVatArray(){let vatArray={};this.items().forEach((tr)=>{const row=tr._row;const key=row.vat();if(!key)return;const vatValue=(row.total()*row.vat())/100;if(vatArray[key])vatArray[key]+=vatValue;else vatArray[key]=vatValue;vatArray[key]=vatArray[key].toFixed(2);if(vatArray[key]==="0.00")delete vatArray[key]});return vatArray}
