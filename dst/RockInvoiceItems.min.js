...
wakeup(){const data=JSON.parse(this.textarea.value);if(data.items){data.items.forEach((item)=>{this.addRow().setData(item.net,item.vat,item.quantity)})}}}
class Item{constructor(field,addedItem){this.field=field;this.tr=addedItem||field.li.querySelector("tbody tr:last-child");this.deleteButton=this.tr.querySelector(".delete-row");this.cloneButton=this.tr.querySelector(".clone-row");this.itemTotal=this.tr.querySelector(".total > strong");this.tr._item=this;this.deleteButton.addEventListener("click",(e)=>{e.preventDefault();this.deleteItem(this.deleteButton)});this.cloneButton.addEventListener("click",(e)=>{e.preventDefault();this.clone()});this.selectOnClick();this.monitorInputs();this.update()}
...
setData(net,vat,quantity){this.tr.querySelector(".net input").value=net;this.tr.querySelector(".vat input").value=vat;this.tr.querySelector(".quantity input").value=quantity;this.update()}
toArray(){return{text:"TBD",net:this.net(),vat:this.vat(),quantity:this.quantity(),}}
