...
getVatArray(){let vatArray={};this.items().forEach((tr)=>{const row=tr._row;const key=row.vat();if(!key)return;const vatValue=(row.total()*row.vat())/100;if(vatArray[key])vatArray[key]+=vatValue;else vatArray[key]=vatValue});Object.keys(vatArray).forEach((key)=>{vatArray[key]=Number(vatArray[key]).toFixed(2);if(vatArray[key]==="0.00")delete vatArray[key]});return vatArray}
...
itemsJSON(){return JSON.stringify(Array.from(this.items()).map((tr)=>{return{text:"TBD",net:tr.querySelector(".net input").value,vat:tr.querySelector(".vat input").value,quantity:tr.querySelector(".quantity input").value,}}))}
...
this.sleep()}
updateVatTotals(){let vatArray=this.getVatArray();this.totalstable.querySelectorAll("tr.vattotal:not([hidden])").forEach((tr)=>tr.remove());const tpl=this.totalstable.querySelector("tr.vattotal[hidden]");for(const[key,value]of Object.entries(vatArray)){const clone=tpl.cloneNode(!0);clone.querySelector(".rate").textContent=key+"%";clone.querySelector(".value").textContent=value;clone.removeAttribute("hidden");const last=this.totalstable.querySelector("tbody tr:last-child");this.totalstable.querySelector("tbody").insertBefore(clone,last)}}}
class Row{constructor(field){this.field=field;this.tr=field.li.querySelector("tbody tr:last-child");this.deleteButton=this.tr.querySelector(".delete-row");this.cloneButton=this.tr.querySelector(".clone-row");this.rowTotal=this.tr.querySelector(".total > strong");this.tr._row=this;this.deleteButton.addEventListener("click",(e)=>{e.preventDefault();this.deleteRow(this.deleteButton)});this.cloneButton.addEventListener("click",(e)=>{e.preventDefault();this.clone()});this.selectOnClick();this.monitorInputs();this.update()}
clone(){const clone=this.tr.cloneNode(!0);this.tr.after(clone);new Row(this.field);this.update()}
